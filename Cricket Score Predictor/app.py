...
with col1:
    batting_team = st.selectbox('Selecting batting team',sorted(teams))
with col2:
    bowling_team = st.selectbox('Selecting bowling team', sorted(teams))
...
if st.button('Predict Score'):
    ...
    input_df = pd.DataFrame(
        {'batting_team': [batting_team], 'bowling_team': [bowling_team], 'city': city, 'current_score': [current_score],
         'balls_left': [balls_left], 'wickets_left': [wickets], 'curr': [curr], 'last_five': [last_five]})
